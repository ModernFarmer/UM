function _clearTime(i){      //按顺序清除i个定时器
	for(var j=1; j<i; j++){
		clearInterval(j);
	}
};

function _RA(str, Arr){     //移除数组Arr中的指定元素str
...
function _getClient(event){     //获取鼠标到页面的距离>>>>>>>必须放在(鼠标事件)内部才能生效
...
function _isJson(obj){             //判断一个对象是否为json对象,返回布尔值
	var _isjson = typeof(obj) == "object" && Object.prototype.toString.call(obj).toLowerCase() == "[object object]" && !obj.length; 
	return _isjson;
};
...
function _dateFormat(dateObj, string){          //格式化日期
	let json_inf = {
...
function _getPastMonth(n){        //获取前n个月的日期,格式:'年-月',类型:字符串
	...
	if(m<=n%12){
		let month=12-(n%12-m)>10?12-(n%12-m):'0'+(12-(n%12-m));
		return (y-1-parseInt(n/12))+'-'+month;
	}else{
		let month=m-n%12>10?m-n%12:'0'+(m-n%12);
		return (y-1-parseInt(n/12))+'-'+month;
	};
};
...
function _getPastWeek(n){                      //获取前n周的日期,格式:'年-月-日,年-月-日',类型:字符串
	...
	if(weekDay==0){
		let date_from=new Date(timeChuo-(13+7*(n-1))*24*60*60*1000);
		let date_to=new Date(timeChuo-(7+7*(n-1))*24*60*60*1000);
		let Month_from=(date_from.getMonth()+1)>9?date_from.getMonth()+1:'0'+(date_from.getMonth()+1);
		let Day_from=date_from.getDate()>9?date_from.getDate():'0'+date_from.getDate();
		let Month_to=(date_to.getMonth()+1)>9?date_to.getMonth()+1:'0'+(date_to.getMonth()+1);
		let Day_to=date_to.getDate()>9?date_to.getDate():'0'+date_to.getDate();
		return date_from.getFullYear()+'-'+Month_from+'-'+Day_from+','+date_to.getFullYear()+'-'+Month_to+'-'+Day_to;
	}else{
		let date_from=new Date(timeChuo-(weekDay+6+7*(n-1))*24*60*60*1000);
		let date_to=new Date(timeChuo-(weekDay+7*(n-1))*24*60*60*1000);
		let Month_from=(date_from.getMonth()+1)>9?date_from.getMonth()+1:'0'+(date_from.getMonth()+1);
		let Day_from=date_from.getDate()>9?date_from.getDate():'0'+date_from.getDate();
		let Month_to=(date_to.getMonth()+1)>9?date_to.getMonth()+1:'0'+(date_to.getMonth()+1);
		let Day_to=date_to.getDate()>9?date_to.getDate():'0'+date_to.getDate();
...
function _maxDate(str){     //获取某个月的最大天数.  参数的格式必须为'年-月'或者'年-月-......'
	if(typeof str!=='string' || !str.includes('-')){
		alert('参数错误! _maxDate()方法的参数必须为一个字符串, 其格式必须为 \'年-月\' 或者 \'年-月-xxx\'. 如\'2018-08\', \'2018-8-10 15:5:20\', ...');
		return;
	}
    return str+'-'+new Date(parseInt(str.split('-')[0]),parseInt(str.split('-')[1]),0).getDate();
};

function _ToURL(json){                          //将json数据转换成a='xxx'&b='xxx'&c='xxx'形式的字符串(供ajax方法使用)
    var arr=[];
...
function _ajax(json){
    ...
    if(!json.FormData && !json.FormData_More){
        ...
        switch(json.method){
            case 'get':
                ajaxObj.open('GET',json.url+'?'+_ToURL(json.data), json.async);
                ajaxObj.send();
                break;
            case 'post':
                ajaxObj.open('POST',json.url, json.async);
                ajaxObj.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
                ajaxObj.send(_ToURL(json.data));
                break;
        }
        ...
        for(let i=0; i<json.FormData_More.files.length; i++){
        ...
        if(json.FormData_More.encoded){
            for(let name in json.FormData_More.encoded){
...
_getDomObject.prototype.css=function (typeName, val){    //样式设置.css()方法原型
	...
	if(this.el[0]==undefined){
		if(typeName && _isJson(typeName) && !val && val!=0){
			for(name in typeName){
	    		this.el.style[name]=typeName[name];
			};
		}else if(typeName && (val || val==0) && !_isJson(typeName)){	
...
_getDomObject.prototype.BD=function (eventName,fn){      //事件绑定.BD()方法原型
	if(this.el==undefined)return;
	if(this.el[0]==undefined){
		if(eventName && _isJson(eventName) && !fn){
		...
		}else if(eventName && fn && !_isJson(eventName)){
...
_getDomObject.prototype.MBD=function (eventName,fn){      //移动端事件绑定.MBD()方法原型
	if(this.el==undefined)return;
	if(this.el[0]==undefined){
		if(eventName && _isJson(eventName) && !fn){
			for(name in eventName){
				if(typeof(eventName[name])!=='function'){
					...
					this.el.addEventListener(name,eventName[name],{passive: false});
				};
			};	
		}else if(eventName && fn && !_isJson(eventName)){
			if(typeof(fn)!=='function'){
		...
		if(eventName && _isJson(eventName) && !fn){
			for(name in eventName){
				if(typeof(eventName[name])!=='function'){
					...
					this.each(function(i){
						_this.addEventListener(name,eventName[name],{passive: false});
					});
				};	
			}	
		}else if(eventName && fn && !_isJson(eventName)){	
			if(typeof(fn)!=='function'){
				...
				this.each(function(i){
					_this.addEventListener(eventName,fn,{passive: false});
				});
			};
		}else {
			alert('BD方法：.BD()参数设置错误；.BD()的参数必须是 [字符串+回调函数(或函数名变量)] 或者是 [一个json对象]');
