...
/*function _condense(json){      // 压缩图片
	...
	if(_isArray(file)){    // 判断file是否是一个数组
		...
		for(var i=0; i<um_imgArr.length; i++){
			var promiseObj=new Promise((resolve, reject)=>{
				um_imgArr[i].img.onload=function(){
				    // 图片原始尺寸
				    ...
				    if((maxWidth && maxWidth>0) && (maxHeight && maxHeight>0)){
					    if(originWidth>maxWidth || originHeight>maxHeight){
					    	if(originWidth/originHeight>maxWidth/maxHeight){
					    		um_imgArr[i].target_width=maxWidth;
					    		um_imgArr[i].target_height=Math.ceil(maxWidth*(originHeight/originWidth));
					    	}else{
					    		um_imgArr[i].target_width=Math.ceil(maxHeight*(originWidth/originHeight));
					    		um_imgArr[i].target_height=maxHeight;
					    	};
					    }else{
					    	um_imgArr[i].target_width=originWidth;
					    	um_imgArr[i].target_height=originHeight;
					    };
				    ...
				    // 指定画布大小
				    um_imgArr[i].canvas.width=um_imgArr[i].target_width;
				    um_imgArr[i].canvas.height=um_imgArr[i].target_height;
				    // 画布生成画面
				    um_imgArr[i].context.drawImage(um_imgArr[i].img, 0, 0, um_imgArr[i].target_width, um_imgArr[i].target_height);

				    if(quality){
					    um_imgArr[i].canvas.toBlob(blob=>{
				...
				um_imgArr[i].reader.onload=function(){
				    um_imgArr[i].img.src=this.result;
				};

				um_imgArr[i].reader.readAsDataURL(um_imgArr[i].file);
			});
